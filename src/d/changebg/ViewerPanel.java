...
/* The Kinect RGB image is modified so that only the user (or users)
   ...
   A UserGenerator node supplies the location of the users in
   the corresponding depth image. Also, the depth and RGB images are aligned using
   the getAlternativeViewpointCapability() method. Then the positions in the
   user generator image which are not users (i.e. == 0) are set to be transparent
   in the RGB image. 
*/
...
public class ViewerPanel extends JPanel implements Runnable
{
  ...
  public ViewerPanel(String backFnm)
  {
  ...
  */
  ...
  // draw background then the camera image
  { 
