...
// Andrew Davison, ad@fivedots.coe.psu.ac.th, July 2012
...
/* Class for accessing the Kinect Sensor's motor
   ...
   CHANGES
     July 2012: convertAcceleration() written by Fabian Saccilotto 
    (fabian.saccilotto@ntb.ch), which fixes a masking bug in my original code
*/
...
public class MotorCommunicator
{
  ...
  */
  ...
  // acceleration counts
  {
    ...
    accel[0] = convertAcceleration(buf, 2, 3);
    accel[1] = convertAcceleration(buf, 4, 5);
    accel[2] = convertAcceleration(buf, 6, 7);
    return accel;
  }  // end of getAccel()

  
  private int convertAcceleration(byte[] buf, int firstByte, int secondByte)
  // written by Fabian Saccilotto (fabian.saccilotto@ntb.ch), July 2012 -- thanks
  {
    short tmp = (short) (buf[firstByte] << 8);
    tmp |= buf[secondByte] & 0xFF;
    return tmp;
  }  // end of convertAcceleration()
  ...
  /* Connect to the Kinect, changing its LED to blinking red/orange.
     Wait for the user to input an angle (in the range +/- 31 degrees),
     rotate the Kinect and report various state information.
     The user finishes by typing 'q', and the Kinect light goes back
     to blinking green.
  */
  {
